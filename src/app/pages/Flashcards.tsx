...
import { ArrowLeft, BookOpen, ChevronLeft, ChevronRight, Plus, RefreshCcw, Search } from "lucide-react";
...
export function Flashcards() {
  ...
  const [draftCards, setDraftCards] = useState<DraftCard[]>([createEmptyDraftCard(), createEmptyDraftCard()]);
  ...
  const [quickCardType, setQuickCardType] = useState<"term_definition" | "qa" | "cloze">("term_definition");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [sets, docs] = await Promise.all([listFlashcardSets(50, 0), listDocuments(50, 0)]);
        setSetList(sets);
        setDocuments(docs);
        ...
        const firstCompleted = sets.find((item) => item.generation_status === "completed")?.set_id;
  ...
  const addEditCard = () => {
    setEditCards((prev) => [...prev, { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, front: "", back: "" }]);
  };
  ...
  const updateEditCard = (id: string, field: "front" | "back", value: string) => {
    setEditCards((prev) => prev.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
  };
  ...
  const handleSaveEditSet = () => {
    ...
    const run = async () => {
      try {
        ...
        const editedCardIds = new Set(editCardsWithContent.filter((item) => item.card_id).map((item) => item.card_id as string));
  ...
  const updateDraftCard = (id: string, field: "front" | "back", value: string) => {
    setDraftCards((prev) => prev.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-8">
          <div className="space-y-6">
            <div>
              <label className="mb-2 block text-base font-semibold text-slate-800">Tiêu đề bộ thẻ *</label>
              <input
            ...
            <div>
              <label className="mb-2 block text-base font-semibold text-slate-800">Mô tả (Tùy chọn)</label>
              <textarea
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-base font-semibold text-slate-800">Danh mục</label>
                <input
              ...
              <div>
                <label className="mb-2 block text-base font-semibold text-slate-800">Tài liệu liên kết</label>
                <select
                ...
                >
                  ...
                  {!documents.length && <option value="">Không có tài liệu</option>}
                  {documents.map((doc) => (
            ...
            <div className="space-y-4 pt-2">
              <p className="text-2xl font-bold text-slate-900">Các thẻ ({draftCards.length})</p>
                ...
                {draftCards.map((draft, index) => (
                  <div key={draft.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div className="mb-4 flex items-center justify-between border-b border-slate-200 pb-3">
                      <p className="text-xl font-semibold text-slate-500">Thẻ {index + 1}</p>
                      <button
                        type="button"
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <label className="mb-2 block text-base font-semibold text-slate-700">Thuật ngữ</label>
                        <input
                          value={draft.front}
                          onChange={(event) => updateDraftCard(draft.id, "front", event.target.value)}
                      ...
                      <div>
                        <label className="mb-2 block text-base font-semibold text-slate-700">Định nghĩa</label>
                        <input
                          value={draft.back}
                          onChange={(event) => updateDraftCard(draft.id, "back", event.target.value)}
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-8">
          <div className="space-y-6">
            ...
            <div>
              <label className="mb-2 block text-base font-semibold text-slate-800">Mô tả</label>
              <textarea
            ...
            <div className="space-y-4">
              <p className="text-xl font-bold text-slate-900">Nội dung thẻ ({editCards.length})</p>
              {editCards.map((item, index) => (
                <div key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <p className="font-semibold text-slate-700">Thẻ {index + 1}</p>
                    <button
                      type="button"
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <input
                      value={item.front}
                      onChange={(event) => updateEditCard(item.id, "front", event.target.value)}
                      placeholder="Thuật ngữ"
                      className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700 outline-none focus:border-[#00A651]"
                    />
                    <input
                      value={item.back}
                      onChange={(event) => updateEditCard(item.id, "back", event.target.value)}
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3 border-b border-green-100 pb-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Tạo, chỉnh sửa và ôn tập các bộ thẻ ghi nhớ của bạn.</p>
        </div>
      ...
      <div className="-mt-2 flex flex-wrap items-center gap-3">
        ...
        {!selectedDocumentId && <p className="text-xs text-slate-500">Chưa có tài liệu để tạo thẻ bằng AI.</p>}
      </div>

      {loadingSets && <p className="text-sm text-slate-500">Đang tải thư viện flashcard...</p>}
      {!loadingSets && filteredSets.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center">
          <p className="text-slate-600">Chưa có bộ thẻ nào.</p>
          <Link href="/documents" className="mt-2 inline-block text-sm font-semibold text-[#00A651] hover:text-[#008f45]">
      ...
      {visibleSets.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-end gap-2">
            <button
              type="button"
              onClick={() => setLibraryStartIndex((prev) => Math.max(0, prev - PAGE_SIZE))}
            ...
            <button
              type="button"
              onClick={() => setLibraryStartIndex((prev) => Math.min(prev + PAGE_SIZE, Math.max(0, filteredSets.length - PAGE_SIZE)))}
            ...
            return (
              ...
              >
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500">{setItem.studied_cards}/{setItem.card_count}</span>
                </div>
                <h3 className="line-clamp-2 text-lg font-bold text-slate-900">{setItem.title}</h3>
      ...
      {isStudyOpen && (
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-lg font-semibold text-slate-900">
                {studySource === "today" ? "Từ cần học hôm nay" : setDetail?.title ?? "Bộ ôn tập"}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${
                  studySource === "set" ? "bg-[#00A651] text-white" : "bg-slate-100 text-slate-600"
                }`}
              >
                Bộ đang chọn
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${
                  studySource === "today" ? "bg-[#00A651] text-white" : "bg-slate-100 text-slate-600"
                }`}
              >
          ...
          {studySource === "set" && loadingSetData && <span className="mb-3 block text-sm text-slate-500">Đang tải bộ thẻ...</span>}
          {studySource === "today" && loadingDueToday && <span className="mb-3 block text-sm text-slate-500">Đang tải danh sách hôm nay...</span>}

          {studySource === "set" && (
          <div className="mb-5 grid grid-cols-1 gap-2 md:grid-cols-[0.7fr_1fr_1fr_auto]">
            <select
              value={quickCardType}
              onChange={(event) => setQuickCardType(event.target.value as "term_definition" | "qa" | "cloze")}
            ...
            <button
              ...
              disabled={quickAdding || !selectedSetId || !quickFront.trim() || !quickBack.trim()}
          ...
          {!loadingSetData && studySource === "set" && !card && <p className="text-sm text-slate-500">Bộ thẻ này chưa có thẻ để ôn tập.</p>}
          {!loadingDueToday && studySource === "today" && !card && <p className="text-sm text-slate-500">Hôm nay không có thẻ nào đến hạn ôn tập.</p>}
          ...
          {card && (
            <div className="flex flex-col items-center">
              ...
              >
                ...
                >
                  <div className="absolute flex h-full w-full flex-col items-center justify-center rounded-2xl border border-emerald-200 bg-white p-8 text-center [backface-visibility:hidden]">
                    <span className="absolute left-5 top-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Front</span>
                    <p className="text-2xl font-bold text-slate-900">{card.front}</p>
                  ...
                  <div className="absolute flex h-full w-full flex-col items-center justify-center rounded-2xl border border-emerald-300 bg-emerald-100 p-8 text-center text-emerald-950 [backface-visibility:hidden] [transform:rotateX(180deg)]">
                    <span className="absolute left-5 top-4 text-xs font-semibold uppercase tracking-wide text-emerald-900">Back</span>
              ...
              {cardStatusLabel && <p className="mt-3 text-xs text-slate-500">Trạng thái: {cardStatusLabel}</p>}
